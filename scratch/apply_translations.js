...
// Helper to escape single quotes inside string values for JS object keys/values
function escapeSingleQuotes(str) {
  return str.replace(/'/g, "\\'");
}

// Generate the lines to inject
const enLines = [];
const frLines = [];
const esLines = [];
...
const enAnchor = '  en: {\n';
const enPos = i18nContent.indexOf(enAnchor);
if (enPos === -1) {
  console.error("Could not find English block start!");
  process.exit(1);
}
...
// Insert into French block
const frAnchor = '  fr: {\n';
const frPos = i18nContent.indexOf(frAnchor);
if (frPos === -1) {
  console.error("Could not find French block start!");
  process.exit(1);
}
...
// Insert into Spanish block
const esAnchor = '  es: {\n';
const esPos = i18nContent.indexOf(esAnchor);
if (esPos === -1) {
  console.error("Could not find Spanish block start!");
  process.exit(1);
}
...
console.log("Successfully injected all translations into i18n.js!");
