document.addEventListener('DOMContentLoaded', () => {
  ...
  const initAdModalControls = () => {
    ...
    const updateAdPrice = () => {
      ...
      if (adDaysVal) {
        adDaysVal.textContent = days === 1 ? tText('1 jour') : `${days} ${tText('jours')}`;
      }
      if (adTotalPriceVal) {
    ...
    const submitAdForm = async (event) => {
      ...
      if (!title || !description || !adUrl || !days) {
        const message = tText('Complétez le titre, la description, le lien et la durée.');
      ...
      try {
        ...
        const message = tText('Entrez une URL valide, par exemple https://votre-site.com.');
        setAdFormStatus(message);
        showAdToast(message);
        urlInput?.focus();
        return;
      ...
      if (!adImageInput?.files?.length) {
        const message = tText('Ajoutez une image publicitaire avant de publier.');
        setAdFormStatus(message);
        showAdToast(message);
        adImageInput?.click();
        return;
      ...
      if (submitBtn) {
        ...
        submitBtn.innerHTML = `<i class="spinner" style="display:inline-block; width:12px; height:12px; border:2px solid white; border-top-color:transparent; border-radius:50%; animation:spin 0.6s linear infinite; margin-right:6px;"></i> ${tText('Envoi en cours...')}`;
      }
  ...
  const parseStructuredChatContent = (content) => {
  ...
  const getChatMessagePreviewText = (message) => {
    ...
    const structuredContent = parseStructuredChatContent(content);
    const gameData = structuredContent && structuredContent.type === 'game_invitation' ? structuredContent : null;
    const sharedPostData = structuredContent && structuredContent.type === 'shared_post' ? structuredContent : null;
    const sharedReelData = structuredContent && structuredContent.type === 'shared_reel' ? structuredContent : null;
    ...
    if (gameData && gameData.type === 'game_invitation') {
      ...
      const gameKey = String(gameData.game || '').toLowerCase();
      const gameLabel = gameNames[gameKey] || gameData.game || 'Jeu';
    ...
    if (sharedPostData) {
      const post = sharedPostData.post && typeof sharedPostData.post === 'object' ? sharedPostData.post : {};
      ...
      if (excerpt) return `Post partage : ${excerpt.length > 80 ? `${excerpt.slice(0, 77)}...` : excerpt}`;
      if (authorName) return `Post partage de ${authorName}`;
      return 'Post partage';
    }
    if (sharedReelData) {
      const reel = sharedReelData.reel && typeof sharedReelData.reel === 'object' ? sharedReelData.reel : {};
      ...
      if (excerpt) return `Short partage : ${excerpt.length > 80 ? `${excerpt.slice(0, 77)}...` : excerpt}`;
  ...
  const renderSharedContentBubble = (sharedContentData) => {
    ...
    const contentData = isReel
      ...
      : (sharedContentData?.post && typeof sharedContentData.post === 'object' ? sharedContentData.post : {});
    const authorName = escapeHtml(contentData.authorName || contentData.authorUsername || (isReel ? 'Short partage' : 'Publication partagee'));
    const authorUsername = escapeHtml(contentData.authorUsername || '');
    const authorAvatar = escapeHtml(contentData.authorAvatar || '/assets/avatar_placeholder.jpg');
    const excerpt = escapeHtml(String(contentData.excerpt || contentData.caption || contentData.content || '').trim() || (isReel ? 'Ouvrez le short partage.' : 'Ouvrez la publication partagee.'));
    const imageUrl = escapeHtml(contentData.previewImageUrl || contentData.imageUrl || contentData.thumbnailUrl || '');
    const videoUrl = escapeHtml(contentData.previewVideoUrl || contentData.videoUrl || '');
    const mediaType = String(contentData.mediaType || (videoUrl ? 'video' : 'image')).toLowerCase();
    const targetUrl = escapeHtml(
      sharedContentData?.appUrl
      || sharedContentData?.shareUrl
      || (isReel
        ? `${window.location.origin}/?view=shorts#reel-${Number(contentData.id || 0)}`
        : `${window.location.origin}/?shared_post=${Number(contentData.id || 0)}#post-${Number(contentData.id || 0)}`)
    );
    const contentId = Number(contentData.id || 0);
    const snapshot = contentData.snapshot && typeof contentData.snapshot === 'object' ? contentData.snapshot : null;

    if (!isReel && snapshot?.kind === 'post') {
      const likeCount = escapeHtml(String(snapshot.likeCount ?? '0'));
      const commentCount = escapeHtml(String(snapshot.commentCount ?? '0'));
      const shareCount = escapeHtml(String(snapshot.shareCount ?? '0'));
      const tradeLabel = snapshot.tradeLabel ? `
        <button class="post-action-btn trade-btn" type="button">
          <i data-lucide="repeat"></i>
          <span class="action-label"><span class="btn-text-label">${escapeHtml(snapshot.tradeLabel)}</span></span>
        </button>
      ` : '';
      return `
        <div class="chat-msg-bubble chat-shared-post-bubble chat-shared-snapshot-bubble" tabindex="0" role="link" data-shared-target-url="${targetUrl}" data-shared-content-id="${contentId || ''}" data-shared-content-type="post">
          <div class="chat-facebook-share-shell">
            <div class="chat-shared-capture-badge chat-facebook-share-badge">
              <i data-lucide="share-2" style="width: 11px; height: 11px;"></i>
              <span>Post partage</span>
            </div>
            <article class="post-card chat-shared-feed-card chat-shared-feed-card-post">
              <header class="post-header">
                <div class="post-author-info">
                  <div class="post-author-link chat-shared-static-author">
                    <div class="avatar author-avatar">
                      <img src="${authorAvatar}" alt="${authorName}">
                    </div>
                    <div class="author-details">
                      <h3 class="author-name">${snapshot.authorNameHtml || authorName}</h3>
                      <span class="post-time">${escapeHtml(snapshot.timeText || 'Publication')}</span>
                    </div>
                  </div>
                </div>
              </header>
              <div class="post-content">${snapshot.contentHtml || ''}</div>
              <footer class="post-footer">
                <div class="post-actions">
                  <button class="post-action-btn like-btn" type="button">
                    <i data-lucide="heart"></i>
                    <span class="action-label"><span class="like-count">${likeCount}</span><span class="btn-text-label"> Like</span></span>
                  </button>
                  <button class="post-action-btn comment-btn" type="button">
                    <i data-lucide="message-circle"></i>
                    <span class="action-label"><span class="comment-count">${commentCount}</span><span class="btn-text-label"> Comment</span></span>
                  </button>
                  <button class="post-action-btn share-btn" type="button">
                    <i data-lucide="share-2"></i>
                    <span class="action-label"><span class="share-count">${shareCount}</span><span class="btn-text-label"> Share</span></span>
                  </button>
            ...
            <div class="chat-facebook-share-cta">
              <span class="chat-facebook-share-domain">TrasX</span>
              <span class="chat-facebook-share-open">Voir la publication</span>
            </div>
          </div>
        </div>
      `;
    ...
    if (isReel && snapshot?.kind === 'reel') {
      const likeCount = escapeHtml(String(snapshot.likeCount ?? '0'));
      const commentCount = escapeHtml(String(snapshot.commentCount ?? '0'));
      const shareCount = escapeHtml(String(snapshot.shareCount ?? '0'));
      const mediaHtml = mediaType === 'video' && videoUrl
        ? `<video src="${videoUrl}" muted playsinline preload="metadata" poster="${imageUrl || ''}"></video>`
        : (imageUrl ? `<img src="${imageUrl}" alt="${authorName}">` : '<div class="chat-shared-reel-fallback"></div>');
      return `
        <div class="chat-msg-bubble chat-shared-post-bubble chat-shared-snapshot-bubble is-reel" tabindex="0" role="link" data-shared-target-url="${targetUrl}" data-shared-content-id="${contentId || ''}" data-shared-content-type="reel">
          <div class="chat-shared-reel-card">
            <div class="chat-shared-capture-badge chat-tiktok-share-badge">
              <i data-lucide="share-2" style="width: 11px; height: 11px;"></i>
              <span>Short partage</span>
            </div>
            <div class="chat-shared-reel-media">${mediaHtml}</div>
            <div class="chat-shared-reel-play">
              <i data-lucide="play" style="width: 16px; height: 16px; fill: currentColor;"></i>
            </div>
            <div class="chat-shared-reel-overlay">
              <div class="chat-shared-reel-author">
                <img src="${authorAvatar}" alt="${authorName}">
                <div>
                  <div class="chat-shared-reel-author-name">${snapshot.authorNameHtml || authorName}</div>
                  ${authorUsername ? `<span>@${authorUsername}</span>` : ''}
                </div>
              </div>
              <p class="chat-shared-reel-caption">${snapshot.captionHtml || excerpt}</p>
              <div class="chat-shared-reel-stats chat-shared-reel-stats-vertical">
                <span><i data-lucide="heart" style="width: 12px; height: 12px;"></i><strong>${likeCount}</strong></span>
                <span><i data-lucide="message-square" style="width: 12px; height: 12px;"></i><strong>${commentCount}</strong></span>
                <span><i data-lucide="share-2" style="width: 12px; height: 12px;"></i><strong>${shareCount}</strong></span>
              </div>
            </div>
          </div>
        </div>
    ...
    if (mediaType === 'video' && videoUrl) {
      mediaHtml = `
        <div class="chat-shared-post-media chat-shared-post-media-video">
          <video src="${videoUrl}" muted playsinline preload="metadata"></video>
        </div>
      `;
    } else if (imageUrl) {
      mediaHtml = `
        <div class="chat-shared-post-media">
    ...
    return `
      <div class="chat-msg-bubble chat-shared-post-bubble" tabindex="0" role="link" data-shared-target-url="${targetUrl}" data-shared-content-id="${contentId || ''}" data-shared-content-type="${isReel ? 'reel' : 'post'}">
        <div class="chat-shared-post-card">
          <div class="chat-shared-post-kicker">
            <i data-lucide="share-2" style="width: 12px; height: 12px;"></i>
            <span>${isReel ? 'Short partage' : 'Post partage'}</span>
          </div>
          <div class="chat-shared-post-author">
            <img src="${authorAvatar}" alt="${authorName}">
            <div>
          ...
          ${mediaHtml}
          <p class="chat-shared-post-excerpt">${excerpt}</p>
          <span class="chat-shared-post-link">${isReel ? 'Voir le short' : 'Voir la publication'}</span>
        </div>
      </div>
    `;
  };
  ...
  const renderChatMessageHtml = (message, msgType) => {
    ...
    let gameData = null;
    const structuredContent = parseStructuredChatContent(rawContent);
    const isSharedSnapshot = structuredContent && (structuredContent.type === 'shared_post' || structuredContent.type === 'shared_reel');
    if (structuredContent && structuredContent.type === 'game_invitation') {
      gameData = structuredContent;
    ...
    } else if (structuredContent && (structuredContent.type === 'shared_post' || structuredContent.type === 'shared_reel')) {
    ...
    return `
      <div class="chat-msg-wrapper ${msgType}${isSharedSnapshot ? ' chat-msg-wrapper-shared' : ''}" data-message-id="${escapeHtml(message?.id ?? '')}" data-message-status="${escapeHtml(status || '')}">
        ${bubbleContentHtml}
        ${attachmentHtml}
  ...
  const createWatermarkedVideoDownload = async (button) => {
    ...
    try {
      ...
      const drawFrame = () => {
        ...
        ctx.fillText("TrasX", Math.round(x + padding), Math.round(y + 16 * scaleFactor));
      ...
      downloadBlob(blob, `trasx-video-watermarked-${Date.now()}.${recordingFormat.ext}`);
  ...
  const platformEndAudioSrc = '/assets/platform-end-chime.mp3';

  const ensurePlatformEndAudioUnlocked = () => {
    try {
      if (!platformEndAudio) {
        platformEndAudio = new Audio(platformEndAudioSrc);
        platformEndAudio.preload = 'auto';
        platformEndAudio.volume = 0.88;
      }
      platformEndAudio.load();
      platformEndAudioUnlocked = true;
    } catch (error) {
      console.warn('platform end audio unlock failed:', error);
    }
  };
  ...
  const playPlatformEndSound = () => {
    try {
      ...
      const playback = platformEndAudio.cloneNode();
      playback.volume = 0.88;
      playback.currentTime = 0;
      playback.play().catch((error) => {
        console.warn('platform end sound playback failed:', error);
      });
    } catch (error) {
      console.warn('platform end sound failed:', error);
    }
  };

  const ensureMediaEndOverlay = (container, overlayClass, logoClass) => {
    if (!container) return null;
    let overlay = container.querySelector(`.${overlayClass}`);
    if (!overlay) {
  ...
  const setMediaEndOverlayUsername = (overlay, username) => {
    if (!overlay) return;
    const usernameNode = overlay.querySelector('.post-video-end-username, .reel-end-username');
    if (!usernameNode) return;
    const cleanUsername = String(username || 'user').trim().replace(/^@+/, '') || 'user';
    usernameNode.textContent = `@${cleanUsername}`;
  };

  const createMediaEndOverlayController = (overlay) => {
  ...
  const initCustomVideoPlayers = (root = document) => {
    root.querySelectorAll('.post-video-shell:not([data-video-ready])').forEach(shell => {
      ...
      const endOverlay = ensureMediaEndOverlay(shell, 'post-video-end-overlay', 'post-video-end-logo');
      setMediaEndOverlayUsername(
        endOverlay,
        downloadBtn?.dataset.videoAuthor
          || shell.closest('.post-card')?.querySelector('.post-author-link')?.getAttribute('href')?.split('/').pop()
          || 'user'
      );
  ...
  const buildSharedPostPayload = (postCard, postId, shareUrl) => {
    ...
    const authorEl = postCard.querySelector('.author-name');
    const avatarEl = postCard.querySelector('.author-avatar img');
    const authorLink = postCard.querySelector('.post-author-link');
    const excerptEl = postCard.querySelector('.post-text, .post-bg-container p, .post-content p');
    const previewMediaEl = postCard.querySelector('.post-content img, .post-content video');
    const postContentEl = postCard.querySelector('.post-content');
    ...
    const tradePriceEl = postCard.querySelector('.trade-price-val');
    const postTimeEl = postCard.querySelector('.post-time');
    const authorHref = authorLink?.getAttribute('href') || '';
    const authorUsername = authorHref.includes('/profile/u/') ? authorHref.split('/profile/u/')[1]?.split('?')[0] || '' : '';

    return {
      type: 'shared_post',
      shareUrl,
      appUrl: `${window.location.origin}/?shared_post=${Number(postId || 0)}#post-${Number(postId || 0)}`,
      post: {
        id: Number(postId || 0),
        authorName: authorEl?.textContent?.trim() || 'Publication',
        authorUsername: decodeURIComponent(authorUsername || '').trim(),
        authorAvatar: avatarEl?.src || '/assets/avatar_placeholder.jpg',
        excerpt: formatShareSnippet(excerptEl?.textContent || postCard.querySelector('.post-content')?.textContent || ''),
        previewImageUrl: previewMediaEl?.getAttribute('src') || previewMediaEl?.getAttribute('poster') || '',
        mediaType: previewMediaEl?.tagName?.toLowerCase() === 'video' ? 'video' : 'image',
        snapshot: {
          kind: 'post',
          authorNameHtml: authorEl?.innerHTML || (authorEl?.textContent?.trim() || 'Publication'),
          timeText: postTimeEl?.textContent?.trim() || '',
          contentHtml: postContentEl?.innerHTML || '',
          ...
          tradeLabel: tradePriceEl?.textContent?.trim() ? `Trade (${tradePriceEl.textContent.trim()})` : ''
        }
      }
    };
  };

  const buildSharedReelPayload = (reelCard, reelId) => {
    if (!reelCard) return null;
    const authorMetaHeading = reelCard.querySelector('.reel-details-overlay h4');
    const authorAvatar = reelCard.querySelector('.reel-details-overlay .avatar img');
    const captionEl = reelCard.querySelector('.reel-caption-text');
    const visualEl = reelCard.querySelector('.reel-video');
    const authorUsername = String(reelCard.getAttribute('data-author-username') || '').trim();
    const authorNameRaw = String(authorMetaHeading?.textContent || authorUsername || 'Short').trim();
    const authorName = authorNameRaw.split('•')[0].replace(/\s+/g, ' ').trim() || authorUsername || 'Short';
    const authorHeadingClone = authorMetaHeading ? authorMetaHeading.cloneNode(true) : null;
    authorHeadingClone?.querySelector('.reel-follow-toggle-btn')?.remove();
    const likeCountEl = reelCard.querySelector('.reel-like-btn .like-count');
    const commentCountEl = reelCard.querySelector('.reel-comment-btn .comment-count');
    ...
    return {
      type: 'shared_reel',
      appUrl: `${window.location.origin}/?view=shorts&shared_reel=${Number(reelId || 0)}#reel-${Number(reelId || 0)}`,
      reel: {
        id: Number(reelId || 0),
        authorName,
        authorUsername,
        authorAvatar: authorAvatar?.src || '/assets/avatar_placeholder.jpg',
        caption: formatShareSnippet(captionEl?.textContent || ''),
        previewImageUrl: mediaType === 'image' ? mediaSource : '',
        previewVideoUrl: mediaType === 'video' ? mediaSource : '',
        mediaType,
        snapshot: {
          kind: 'reel',
          authorNameHtml: authorHeadingClone?.innerHTML || authorName,
          captionHtml: captionEl?.innerHTML || formatShareSnippet(captionEl?.textContent || ''),
          likeCount: likeCountEl?.textContent?.trim() || '0',
          commentCount: commentCountEl?.textContent?.trim() || '0',
          shareCount: shareCountEl?.textContent?.trim() || '0'
        }
  ...
  const focusSharedReelTarget = (reelId, attempt = 0) => {
    const numericReelId = Number(reelId || 0);
    if (!numericReelId) return false;

    if (!document.body.classList.contains('viewing-shorts') && typeof showShortsView === 'function') {
      showShortsView();
    }

    const cards = Array.from(document.querySelectorAll('.reels-feed .reel-card'));
    const targetCard = document.getElementById(`reel-${numericReelId}`);
    ...
    cards.slice(0, targetIndex + 1).forEach((card) => {
      card.classList.remove('progressive-hidden');
    });

    window.requestAnimationFrame(() => {
      if (typeof scrollToReelIndex === 'function') {
        scrollToReelIndex(targetIndex);
      } else {
        targetCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
      }
      targetCard.style.boxShadow = '0 0 25px 5px var(--primary)';
      window.setTimeout(() => {
  ...
  const incrementReelShareUi = (reelId) => {
    const numericReelId = Number(reelId || 0);
    if (!numericReelId) return;
    const shareBtns = document.querySelectorAll(`.reel-share-btn[data-reel-id="${numericReelId}"]`);
    shareBtns.forEach((reelShareBtn) => {
      const countSpan = reelShareBtn.querySelector('.share-count');
      if (!countSpan) return;
      const rawCount = String(reelShareBtn.getAttribute('data-shares') || countSpan.textContent || '0').replace(/[^\d.-]/g, '');
      let count = parseInt(rawCount, 10);
      if (!Number.isFinite(count)) count = 0;
      count += 1;
      reelShareBtn.setAttribute('data-shares', String(count));
      countSpan.textContent = formatNumber(count);
    });
  };

  const openSharedContentTarget = (targetUrl) => {
    const href = String(targetUrl || '').trim();
    if (!href) return;
    try {
      const parsedUrl = new URL(href, window.location.origin);
      const sharedReelId = parsedUrl.searchParams.get('shared_reel');
      const reelHashMatch = parsedUrl.hash.match(/^#reel-(\d+)$/);
      const targetReelId = Number(sharedReelId || (reelHashMatch ? reelHashMatch[1] : 0));

      if (targetReelId && parsedUrl.origin === window.location.origin) {
        const canonicalReelUrl = `${window.location.origin}/?view=shorts&shared_reel=${targetReelId}#reel-${targetReelId}`;
        const shouldForceNavigation = window.innerWidth <= 768
          || window.currentView === 'messages'
          || document.body.classList.contains('viewing-messages');

        if (shouldForceNavigation) {
          window.location.href = canonicalReelUrl;
          return;
        }

        window.history.pushState({}, '', canonicalReelUrl);
        const didFocusTarget = focusSharedReelTarget(targetReelId);
        if (didFocusTarget) {
          return;
        }
      ...
      console.warn('openSharedContentTarget parse failed:', error);
    }
    window.location.href = href;
  };

  document.addEventListener('click', (event) => {
    const sharedBubble = event.target.closest('.chat-shared-snapshot-bubble[data-shared-target-url], .chat-shared-post-bubble[data-shared-target-url]');
    if (!sharedBubble) return;
    event.preventDefault();
    openSharedContentTarget(sharedBubble.getAttribute('data-shared-target-url'));
  });
  ...
  socket.on('chat-message-received', (data) => {
    ...
    if (!isOutgoing) {
      triggerNotification(
        ...
        previewText || 'Vous avez reçu un message',
        sender_avatar || '/assets/avatar_placeholder.jpg'
      );
    }
  });
  ...
  if (skeletonFeed) {
    // Small delay so users briefly see the shimmer on slow connections
    setTimeout(() => {
      skeletonFeed.classList.add('hidden');
    }, 350);
  }

  const skeletonStories = document.getElementById('skeletonStories');
  const storiesContainer = document.getElementById('storiesContainer');
  const storiesWrapper = document.getElementById('storiesWrapper');
  if (skeletonStories) {
    const hideStoriesSkeleton = () => {
      skeletonStories.classList.add('hidden');
      if (storiesContainer) {
        storiesContainer.classList.remove('is-loading');
      }
      if (storiesWrapper) {
        storiesWrapper.classList.remove('stories-loading');
      }
    };

    const hasRenderedStories = storiesContainer && storiesContainer.children.length > 0;
    if (hasRenderedStories) {
      setTimeout(hideStoriesSkeleton, 250);
    } else {
  ...
  const shortsPaginationSentinel = document.getElementById('shortsPaginationSentinel');
  const progressiveFeedBatchSize = Math.max(1, Number(window.feedRevealBatchSize || 6));
  const progressiveShortBatchSize = Math.max(1, Number(window.shortRevealBatchSize || 2));

  const revealNextProgressiveItems = (type, batchSize) => {
    const hiddenItems = Array.from(document.querySelectorAll(`[data-progressive-item="${type}"].progressive-hidden`));
    if (!hiddenItems.length) return 0;
    hiddenItems.slice(0, batchSize).forEach((item) => {
      item.classList.remove('progressive-hidden');
    });
    return Math.min(hiddenItems.length, batchSize);
  };

  const bindProgressiveReveal = (sentinel, type, batchSize, options = {}) => {
    if (!sentinel) return;

    let isRevealing = false;
    const observer = new IntersectionObserver((entries) => {
      const entry = entries[0];
      if (!entry || !entry.isIntersecting || isRevealing) return;

      isRevealing = true;
      window.requestAnimationFrame(() => {
        const revealed = revealNextProgressiveItems(type, batchSize);
        if (!revealed) {
          observer.disconnect();
        }
        isRevealing = false;
      ...
      rootMargin: options.rootMargin || '320px 0px 320px 0px',
    ...
    if (notification && notification.message) {
      ...
      if (notification.type !== 'message') {
        triggerNotification(
          ...
          notification.actor_avatar || '/assets/avatar_placeholder.jpg'
        );
      }
    }
  });
  ...
  if (sharePostBtn && postInput) {
    ...
    if (shareSheetModal) {
      ...
      shareSheetModal.addEventListener('click', async (e) => {
        ...
        try {
          const shareText = `Check out this post from TrasX`;
          ...
          if (recipientBtn) {
            ...
            const sharedPayload = buildSharedPostPayload(activeSharePostCard, activeSharePostId, shareUrl);
            ...
            socket.emit('chat-message', {
              ...
              content: JSON.stringify(sharedPayload || {
                type: 'shared_post',
                shareUrl,
                post: { id: Number(activeSharePostId || 0), excerpt: shareText }
              })
            });
  ...
  const sharedReelId = new URLSearchParams(window.location.search).get('shared_reel');
  ...
  const shortsMobileSearchBtn = document.getElementById('shortsMobileSearchBtn');
  const shortsSearchOverlay = document.getElementById('shortsSearchOverlay');
      ...
      return `
        <button type="button" class="shorts-search-result-item" data-reel-id="${reelId}">
          ...
          <div class="shorts-search-result-copy">
            ...
            ${authorUsername ? `<span>@${authorUsername}</span>` : ''}
            ${caption ? `<p>${caption}</p>` : ''}
            ${soundName ? `<small>${soundName}</small>` : ''}
          </div>
        </button>
      `;
    }).join('');
  ...
  const openShortsSearchOverlay = () => {
    ...
    shortsSearchOverlay.setAttribute('aria-hidden', 'false');
    renderShortsSearchResults(shortsSearchInput?.value || '');
    window.setTimeout(() => shortsSearchInput?.focus(), 40);
  };
  ...
  shortsSearchCloseBtn?.addEventListener('click', closeShortsSearchOverlay);
  shortsSearchOverlay?.addEventListener('click', (event) => {
    if (event.target === shortsSearchOverlay) {
  ...
  const getReelEndOverlayController = (card) => {
    ...
    const overlay = ensureMediaEndOverlay(card, 'reel-end-overlay', 'reel-end-logo');
    setMediaEndOverlayUsername(overlay, card.getAttribute('data-author-username') || 'user');
    card.__reelEndOverlayController = createMediaEndOverlayController(overlay);
    return card.__reelEndOverlayController;
  };

  const scheduleReelLoopResume = (card, media) => {
    if (!card || !media) return;
    const resumeDelayMs = 2800;

    if (card.__reelLoopResumeTimer) {
      clearTimeout(card.__reelLoopResumeTimer);
      card.__reelLoopResumeTimer = null;
    }

    pauseMedia(card);
    getReelEndOverlayController(card)?.show();

    card.__reelLoopResumeTimer = window.setTimeout(() => {
      card.__reelLoopResumeTimer = null;
      const refreshedMedia = getMediaElements(card);
      if (!refreshedMedia) return;
      const startVal = Number.isFinite(refreshedMedia.start) ? refreshedMedia.start : 0;
  ...
  const playMedia = (card) => {
    ...
    if (media.type === 'video') {
      ...
      if (!media.main.dataset.hasEndedOverlay) {
  ...
  const pauseMedia = (card) => {
    ...
    getReelEndOverlayController(card)?.hide();
  ...
  if (reelsShareSheet) {
    ...
    reelsShareSheet.addEventListener('click', async (e) => {
      ...
      try {
        const shareText = `Découvrez ce Short sur TrasX`;
        ...
        if (recipientBtn) {
          ...
          const sharedPayload = buildSharedReelPayload(card, reelId);
        ...
        if (socialBtn) {
          ...
          if (platform === 'copy') {
            await navigator.clipboard.writeText(shareUrl);
            showToast(tText('Lien copié dans le presse-papiers !'));
          } else {
            openSocialShare(platform, shareUrl, shareText);
          }
          closeReelsShareSheet();
  ...
  function syncChallengeTypeAvailability() {
    const missAllowed = canCurrentUserCreateMissChallenge();
    document.querySelectorAll('.challenge-type-btn[data-challenge-type="miss"]').forEach((button) => {
      const isLocked = !missAllowed;
      button.dataset.locked = isLocked ? '1' : '0';
      button.style.opacity = isLocked ? '0.55' : '1';
      button.title = isLocked ? 'Seuls les comptes certifies peuvent creer un challenge miss.' : '';
    });
  }
  ...
  function canCurrentUserCreateMissChallenge() {
    const certificationType = String(window.currentUserCertificationType || 'None').trim();
    return certificationType && certificationType !== 'None';
  }

  syncChallengeTypeAvailability();
  ...
  document.querySelectorAll('.challenge-type-btn').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (nextType === 'miss' && !canCurrentUserCreateMissChallenge()) {
        showToast('Seuls les comptes certifies peuvent creer un challenge miss.');
        return;
      }
  ...
  document.getElementById('applyChallengeConfigBtn')?.addEventListener('click', () => {
    ...
    if (draft.type === 'miss' && !canCurrentUserCreateMissChallenge()) {
      return showToast('Seuls les comptes certifies peuvent creer un challenge miss.');
    }
  ...
  function renderHashtagRichText(text) {
    ...
    // Replace hashtags with interactive links
    let parsed = safeText.replace(/#(\w+)/g, (match, tagName) => {
      const lowerTag = tagName.toLowerCase();
      const className = getHashtagColorClass(lowerTag) || 'hashtag-free';
      return `<a href="javascript:void(0)" class="hashtag-link ${className}" data-tag="${escapeHtml(lowerTag)}">${match}</a>`;
    });
    
    // Replace mentions with links to profile page
    parsed = parsed.replace(/@(\w+)/g, (match, username) => {
      return `<a href="/profile/u/${escapeHtml(username)}" class="mention-link" style="color: var(--primary, #3b82f6); font-weight: 600; text-decoration: none; font-style: italic;">${match}</a>`;
    });
  ...
  const initProfileInteractions = () => {
    ...
    const withdrawStepKycRequired = document.getElementById('withdraw-step-kyc-required');
    ...
    // Dedicated Withdrawal KYC System elements
    const withdrawKycForm = document.getElementById('withdrawKycForm');
    const withdrawKycDocument = document.getElementById('withdrawKycDocument');
    const withdrawKycCameraVideo = document.getElementById('withdrawKycCameraVideo');
    const withdrawKycCameraCanvas = document.getElementById('withdrawKycCameraCanvas');
    const withdrawKycSelfiePreview = document.getElementById('withdrawKycSelfiePreview');
    const withdrawKycSelfieImageData = document.getElementById('withdrawKycSelfieImageData');
    const withdrawKycFaceMatchDistance = document.getElementById('withdrawKycFaceMatchDistance');
    const withdrawKycStartCameraBtn = document.getElementById('withdrawKycStartCameraBtn');
    const withdrawKycCaptureSelfieBtn = document.getElementById('withdrawKycCaptureSelfieBtn');
    const withdrawKycRetakeSelfieBtn = document.getElementById('withdrawKycRetakeSelfieBtn');
    const withdrawKycStatusMsg = document.getElementById('withdrawKycStatusMsg');
    const withdrawKycFormError = document.getElementById('withdrawKycFormError');
    const withdrawKycSubmitBtn = document.getElementById('withdrawKycSubmitBtn');

    // New 2-step wizard and success modal elements
    const withdrawStepKycSuccess = document.getElementById('withdraw-step-kyc-success');
    const withdrawKycSuccessContinueBtn = document.getElementById('withdrawKycSuccessContinueBtn');
    const withdrawKycSubstep1 = document.getElementById('withdraw-kyc-substep-1');
    const withdrawKycSubstep2 = document.getElementById('withdraw-kyc-substep-2');
    const withdrawKycGoToStep2Btn = document.getElementById('withdrawKycGoToStep2Btn');
    const withdrawKycBackToStep1Btn = document.getElementById('withdrawKycBackToStep1Btn');

    let withdrawKycCameraStream = null;
    let withdrawKycDocumentObjectUrl = null;
    let withdrawKycFaceModelsPromise = null;
    let withdrawKycComparisonPromise = null;

    const stopWithdrawKycCamera = () => {
      if (withdrawKycCameraStream) {
        withdrawKycCameraStream.getTracks().forEach(track => track.stop());
        withdrawKycCameraStream = null;
      }
      if (withdrawKycCameraVideo) {
        withdrawKycCameraVideo.srcObject = null;
        withdrawKycCameraVideo.style.display = 'none';
      }
    };

    const revokeWithdrawKycDocumentUrl = () => {
      if (withdrawKycDocumentObjectUrl) {
        URL.revokeObjectURL(withdrawKycDocumentObjectUrl);
        withdrawKycDocumentObjectUrl = null;
      }
    };

    const setWithdrawKycPreviewHint = (message) => {
      if (!withdrawKycSelfiePreview) return;
      withdrawKycSelfiePreview.classList.remove('has-image');
      withdrawKycSelfiePreview.innerHTML = `<span>${message}</span>`;
    };

    const setWithdrawKycStatus = (message, isError = false) => {
      if (isError) {
        if (withdrawKycFormError) {
          withdrawKycFormError.textContent = message;
          withdrawKycFormError.style.display = 'block';
        }
        if (withdrawKycStatusMsg) {
          withdrawKycStatusMsg.style.display = 'none';
        }
      } else {
        if (withdrawKycFormError) {
          withdrawKycFormError.style.display = 'none';
        }
        if (withdrawKycStatusMsg) {
          withdrawKycStatusMsg.textContent = message;
          withdrawKycStatusMsg.style.display = 'block';
        }
      }
    };

    const startWithdrawKycCamera = async () => {
      if (!withdrawKycCameraVideo || !navigator.mediaDevices?.getUserMedia) {
        setWithdrawKycPreviewHint(tText('L\'accès à la caméra est requis pour capturer votre selfie en direct.'));
        return;
      }

      try {
        stopWithdrawKycCamera();
        withdrawKycCameraStream = await navigator.mediaDevices.getUserMedia({
          video: {
            facingMode: 'user',
            width: { ideal: 1280 },
            height: { ideal: 720 }
        ...
        withdrawKycCameraVideo.srcObject = withdrawKycCameraStream;
        withdrawKycCameraVideo.style.display = 'block';
        if (withdrawKycSelfiePreview) {
          withdrawKycSelfiePreview.style.display = 'none';
        }
        await withdrawKycCameraVideo.play();
        
        if (withdrawKycStartCameraBtn) withdrawKycStartCameraBtn.style.display = 'none';
        if (withdrawKycCaptureSelfieBtn) withdrawKycCaptureSelfieBtn.style.display = 'inline-flex';
        if (withdrawKycRetakeSelfieBtn) withdrawKycRetakeSelfieBtn.style.display = 'none';
      } catch (error) {
        console.error('Unable to start withdraw KYC camera:', error);
        setWithdrawKycPreviewHint(tText('L\'accès à la caméra a été refusé ou n\'est pas disponible.'));
      }
    };

    const ensureWithdrawKycFaceModels = async () => {
      if (!window.faceapi) {
        return false;
      }

      if (!withdrawKycFaceModelsPromise) {
        setWithdrawKycStatus(tText('Chargement des modèles de reconnaissance faciale...'));
        withdrawKycFaceModelsPromise = Promise.all([
          faceapi.nets.tinyFaceDetector.loadFromUri('/models/face-api'),
          faceapi.nets.faceLandmark68Net.loadFromUri('/models/face-api'),
          faceapi.nets.faceRecognitionNet.loadFromUri('/models/face-api')
        ]).then(async () => {
          ...
          setWithdrawKycStatus(tText('Modèles chargés avec succès.'));
          return true;
        }).catch((error) => {
          console.error('Unable to load face models:', error);
          withdrawKycFaceModelsPromise = null;
          setWithdrawKycStatus(tText('Erreur de chargement des modèles de reconnaissance faciale.'), true);
          return false;
        });
      }

      return withdrawKycFaceModelsPromise;
    };

    const loadWithdrawImageElement = (source) => new Promise((resolve, reject) => {
      const image = new Image();
      image.crossOrigin = 'anonymous';
      image.onload = () => resolve(image);
      image.onerror = reject;
    ...
    const updateWithdrawKycFaceMatch = async () => {
      if (withdrawKycGoToStep2Btn) {
        withdrawKycGoToStep2Btn.disabled = true;
        withdrawKycGoToStep2Btn.style.opacity = '0.5';
        withdrawKycGoToStep2Btn.style.cursor = 'not-allowed';
      }

      if (!withdrawKycDocument?.files?.length || !withdrawKycSelfieImageData?.value || !withdrawKycFaceMatchDistance) {
        return null;
      }

      if (!window.faceapi) {
        setWithdrawKycStatus(tText('La vérification faciale n\'est pas disponible.'), true);
        return null;
      }

      if (withdrawKycComparisonPromise) {
        return withdrawKycComparisonPromise;
      }

      withdrawKycComparisonPromise = (async () => {
        try {
          const modelsReady = await ensureWithdrawKycFaceModels();
          if (!modelsReady) {
            setWithdrawKycStatus(tText('La vérification faciale n\'est pas disponible (échec modèles).'), true);
            return null;
          }

          setWithdrawKycStatus(tText('Analyse en cours : comparaison du visage et du document...'));

          revokeWithdrawKycDocumentUrl();
          withdrawKycDocumentObjectUrl = URL.createObjectURL(withdrawKycDocument.files[0]);

          const [documentImage, selfieImage] = await Promise.all([
            loadWithdrawImageElement(withdrawKycDocumentObjectUrl),
            loadWithdrawImageElement(withdrawKycSelfieImageData.value)
          ]);
          ...
          if (!documentDetection || !selfieDetection) {
            if (withdrawKycFaceMatchDistance) {
              withdrawKycFaceMatchDistance.value = '';
            }
            if (withdrawKycGoToStep2Btn) {
              withdrawKycGoToStep2Btn.disabled = true;
              withdrawKycGoToStep2Btn.style.opacity = '0.5';
            ...
            let reason = tText('Impossible de détecter un visage') + ' ';
            if (!documentDetection && !selfieDetection) {
              reason += tText('sur le document et sur le selfie.');
            } else if (!documentDetection) {
              reason += tText('sur le document d\'identité.');
            } else {
              reason += tText('sur le selfie.');
            }
            setWithdrawKycStatus(reason, true);
            return null;
          }

          const distance = faceapi.euclideanDistance(documentDetection.descriptor, selfieDetection.descriptor);
          if (withdrawKycFaceMatchDistance) {
            withdrawKycFaceMatchDistance.value = distance.toFixed(4);
          }

          if (distance > 0.6) {
            setWithdrawKycStatus(tText("Le visage ne correspond pas au document d'identité (différence faciale : {distance} > 0.60).").replace('{distance}', distance.toFixed(2)), true);
            if (withdrawKycGoToStep2Btn) {
              withdrawKycGoToStep2Btn.disabled = true;
              withdrawKycGoToStep2Btn.style.opacity = '0.5';
              withdrawKycGoToStep2Btn.style.cursor = 'not-allowed';
            ...
            setWithdrawKycStatus(tText("Reconnaissance faciale réussie ! Même personne détectée (différence faciale : {distance} <= 0.60).").replace('{distance}', distance.toFixed(2)));
            if (withdrawKycGoToStep2Btn) {
              withdrawKycGoToStep2Btn.disabled = false;
              withdrawKycGoToStep2Btn.style.opacity = '1';
              withdrawKycGoToStep2Btn.style.cursor = 'pointer';
              withdrawKycGoToStep2Btn.style.background = 'linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)';
              withdrawKycGoToStep2Btn.style.color = '#fff';
            }
          }

          return distance;
        } catch (error) {
          console.error('Unable to compare withdrawal KYC faces:', error);
          if (withdrawKycFaceMatchDistance) {
            withdrawKycFaceMatchDistance.value = '';
          }
          if (withdrawKycGoToStep2Btn) {
          ...
          setWithdrawKycStatus(tText('Erreur lors de la comparaison faciale.'), true);
          return null;
        }
      })().finally(() => {
        withdrawKycComparisonPromise = null;
      });

      return withdrawKycComparisonPromise;
    };

    const captureWithdrawKycSelfie = () => {
      if (!withdrawKycCameraVideo || !withdrawKycCameraCanvas || !withdrawKycSelfieImageData) {
        return;
      }

      const width = withdrawKycCameraVideo.videoWidth || 640;
      const height = withdrawKycCameraVideo.videoHeight || 480;

      withdrawKycCameraCanvas.width = width;
      withdrawKycCameraCanvas.height = height;
      const context = withdrawKycCameraCanvas.getContext('2d');
      if (!context) return;

      context.drawImage(withdrawKycCameraVideo, 0, 0, width, height);
      const dataUrl = withdrawKycCameraCanvas.toDataURL('image/jpeg', 0.88);
      withdrawKycSelfieImageData.value = dataUrl;

      // Show selfie preview
      if (withdrawKycSelfiePreview) {
        withdrawKycSelfiePreview.classList.add('has-image');
        withdrawKycSelfiePreview.innerHTML = `<img src="${dataUrl}" style="width: 100%; height: 200px; object-fit: cover;" alt="Captured selfie">`;
        withdrawKycSelfiePreview.style.display = 'flex';
      }
      
      // Stop camera stream to freeze it
      stopWithdrawKycCamera();

      if (withdrawKycCaptureSelfieBtn) withdrawKycCaptureSelfieBtn.style.display = 'none';
      if (withdrawKycRetakeSelfieBtn) withdrawKycRetakeSelfieBtn.style.display = 'inline-flex';

      updateWithdrawKycFaceMatch();
    };

    const retakeWithdrawKycSelfie = () => {
      if (withdrawKycSelfieImageData) {
        withdrawKycSelfieImageData.value = '';
      }
      if (withdrawKycFaceMatchDistance) {
        withdrawKycFaceMatchDistance.value = '';
      }
      ...
      if (withdrawKycSelfiePreview) {
        withdrawKycSelfiePreview.innerHTML = `<span>${tText("Démarrez votre caméra, puis prenez une photo claire de votre visage.")}</span>`;
        withdrawKycSelfiePreview.classList.remove('has-image');
        withdrawKycSelfiePreview.style.display = 'none';
      }
      startWithdrawKycCamera();
    };

    withdrawKycDocument?.addEventListener('change', () => {
      if (withdrawKycFaceMatchDistance) {
        withdrawKycFaceMatchDistance.value = '';
      }
      if (withdrawKycGoToStep2Btn) {
    ...
    withdrawKycStartCameraBtn?.addEventListener('click', startWithdrawKycCamera);
    withdrawKycCaptureSelfieBtn?.addEventListener('click', captureWithdrawKycSelfie);
    withdrawKycRetakeSelfieBtn?.addEventListener('click', retakeWithdrawKycSelfie);

    const initiateWithdrawal = async () => {
      // Reset steps
      if (withdrawStepKycRequired) withdrawStepKycRequired.style.display = 'none';
      if (withdrawStepKycSuccess) withdrawStepKycSuccess.style.display = 'none';
      ...
      try {
        ...
        if (!data.success) {
          showToast(tText('Erreur de chargement.'));
        ...
        if (data.isFirstWithdrawal && !data.hasPassedKyc) {
          if (withdrawStepKycRequired) {
            ...
            if (withdrawStepKycSuccess) withdrawStepKycSuccess.style.display = 'none';
            if (withdrawKycSubstep1) withdrawKycSubstep1.style.display = 'block';
            if (withdrawKycSubstep2) withdrawKycSubstep2.style.display = 'none';
            if (withdrawKycGoToStep2Btn) {
              withdrawKycGoToStep2Btn.disabled = true;
              withdrawKycGoToStep2Btn.style.opacity = '0.5';
              withdrawKycGoToStep2Btn.style.cursor = 'not-allowed';
            }
            // Start the models and start camera right away to improve UX
            ensureWithdrawKycFaceModels();
            startWithdrawKycCamera();
          } else {
            showToast(tText("Veuillez d'abord passer votre KYC avant d'effectuer votre premier retrait."));
            closeModal('withdrawModal');
          }
          return;
        }
        ...
        if (!data.userWallet) {
          showToast(tText('Veuillez configurer votre adresse de portefeuille (Deposit) avant de retirer.'));
          closeModal('withdrawModal');
          setTimeout(() => {
        ...
        if (isFreshWithdrawDisabled) {
          showToast(tText("Le bouton de retrait est désactivé car votre solde de retrait {balance} est inférieur au montant minimum requis {minAmount}.").replace('{balance}', '$' + freshWithdrawalBalance.toFixed(2)).replace('{minAmount}', '$' + Number(window.minWithdrawalAmount).toFixed(2)));
          closeModal('withdrawModal');
          return;
        }
        ...
        console.error(err);
        showToast(tText('Erreur de connexion.'));
        closeModal('withdrawModal');
      }
    };
    ...
    // Modal MutationObserver to clean up camera on close
    const withdrawModalObserver = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.attributeName === 'class') {
          const isActive = withdrawModal.classList.contains('active');
          if (!isActive) {
            stopWithdrawKycCamera();
            revokeWithdrawKycDocumentUrl();
          }
    ...
    if (withdrawModal) {
      withdrawModalObserver.observe(withdrawModal, { attributes: true });
    }

    // Substep wizard click handlers
    withdrawKycGoToStep2Btn?.addEventListener('click', () => {
      if (withdrawKycSubstep1) withdrawKycSubstep1.style.display = 'none';
      if (withdrawKycSubstep2) withdrawKycSubstep2.style.display = 'block';
    });

    withdrawKycBackToStep1Btn?.addEventListener('click', () => {
      if (withdrawKycSubstep2) withdrawKycSubstep2.style.display = 'none';
      if (withdrawKycSubstep1) withdrawKycSubstep1.style.display = 'block';
    });

    withdrawKycSuccessContinueBtn?.addEventListener('click', async () => {
      if (withdrawStepKycSuccess) withdrawStepKycSuccess.style.display = 'none';
      await initiateWithdrawal();
    });

    withdrawKycForm?.addEventListener('submit', async (event) => {
      event.preventDefault();
      
      if (!withdrawKycDocument?.files?.length) {
        setWithdrawKycStatus(tText('Veuillez téléverser votre document d\'identité.'), true);
        return;
      }

      if (!withdrawKycSelfieImageData?.value) {
        setWithdrawKycStatus(tText('Veuillez capturer votre selfie en direct.'), true);
        return;
      }

      if (withdrawKycSubmitBtn) {
        withdrawKycSubmitBtn.disabled = true;
        withdrawKycSubmitBtn.dataset.originalLabel = withdrawKycSubmitBtn.innerHTML;
        withdrawKycSubmitBtn.innerHTML = `<i data-lucide="loader-circle" class="animate-spin"></i><span>${tText("Analyse et vérification par l'IA...")}</span>`;
        if (window.lucide?.createIcons) {
          window.lucide.createIcons();
        }
      }
      ...
      try {
        let distance = withdrawKycFaceMatchDistance.value;
        if (!distance) {
          distance = await updateWithdrawKycFaceMatch();
        }
        if (distance === null || distance === '' || !Number.isFinite(Number(distance))) {
          setWithdrawKycStatus(tText('Veuillez d\'abord réussir la reconnaissance faciale de l\'étape 1.'), true);
          if (withdrawKycSubstep1) withdrawKycSubstep1.style.display = 'block';
          if (withdrawKycSubstep2) withdrawKycSubstep2.style.display = 'none';
          return;
        }
        ...
        if (Number(distance) > 0.6) {
          setWithdrawKycStatus(tText("La reconnaissance faciale a échoué (différence faciale : {distance} > 0.60). Le selfie ne correspond pas à la photo du document.").replace('{distance}', Number(distance).toFixed(2)), true);
          if (withdrawKycSubstep1) withdrawKycSubstep1.style.display = 'block';
          if (withdrawKycSubstep2) withdrawKycSubstep2.style.display = 'none';
          return;
        }
        ...
        const formData = new FormData();
        formData.append('identity_document', withdrawKycDocument.files[0]);
        formData.append('selfie_image_data', withdrawKycSelfieImageData.value);
        formData.append('face_match_distance', distance);

        const response = await fetch('/api/wallet/withdraw-kyc', {
          method: 'POST',
          body: formData
        });
        ...
        if (data.duplicateBlocked) {
          showToast(data.error || tText("Compte bloqué pour conflit de KYC."));
          setTimeout(() => {
            window.location.href = '/auth/login?error=kyc_conflict_blocked';
          }, 1500);
          return;
        }

        if (data.success) {
          showToast(data.message || tText('Félicitations, votre KYC de retrait a été approuvé !'));
          // Hide KYC step block, show Success Modal block!
          if (withdrawStepKycRequired) withdrawStepKycRequired.style.display = 'none';
          if (withdrawStepKycSuccess) withdrawStepKycSuccess.style.display = 'block';
          if (window.lucide?.createIcons) {
            window.lucide.createIcons();
          }
        } else {
          setWithdrawKycStatus(data.error || tText('Échec de la validation du document par l\'IA.'), true);
        }
      } catch (error) {
        console.error('Unable to run withdraw KYC verification:', error);
        setWithdrawKycStatus(tText('Une erreur est survenue lors de la vérification instantanée du KYC.'), true);
      } finally {
        if (withdrawKycSubmitBtn) {
          withdrawKycSubmitBtn.disabled = false;
          if (withdrawKycSubmitBtn.dataset.originalLabel) {
            withdrawKycSubmitBtn.innerHTML = withdrawKycSubmitBtn.dataset.originalLabel;
            delete withdrawKycSubmitBtn.dataset.originalLabel;
            if (window.lucide?.createIcons) {
              window.lucide.createIcons();
            }
          }
    ...
    if (pinSetupForm) {
      pinSetupForm.addEventListener('submit', async (e) => {
        ...
        if (!/^\d{6}$/.test(pin)) {
          pinSetupError.textContent = tText('Le code secret de retrait doit contenir exactement 6 chiffres.');
        ...
        try {
          ...
          const data = await res.json();
          if (data.success) {
            showToast(tText('Code secret enregistré avec succès !'));
            withdrawStepPinSetup.style.display = 'none';
            ...
            pinSetupError.textContent = data.error || tText('Erreur d\'enregistrement.');
            pinSetupError.style.display = 'block';
          }
        } catch (err) {
          console.error(err);
          pinSetupError.textContent = tText('Erreur serveur.');
          pinSetupError.style.display = 'block';
        }
      });
    }
    ...
    if (withdrawSubmitForm) {
      withdrawSubmitForm.addEventListener('submit', async (e) => {
        ...
        if (isNaN(amount) || amount <= 0) {
          withdrawFormError.textContent = tText('Montant invalide.');
          withdrawFormError.style.display = 'block';
          return;
        }
        
        if (amount > (window.currentUserWithdrawalBalance || 0)) {
          withdrawFormError.textContent = tText('Solde de retrait insuffisant.');
          withdrawFormError.style.display = 'block';
          return;
        }
        
        if (!/^\d{6}$/.test(pin)) {
          withdrawFormError.textContent = tText('Le code secret de retrait doit contenir exactement 6 chiffres.');
          withdrawFormError.style.display = 'block';
          return;
        }
        ...
        try {
          ...
          if (!data.success) {
            ...
            withdrawStatusTitle.textContent = tText('Retrait échoué');
          ...
          withdrawStatusTitle.textContent = tText('Erreur');
          withdrawStatusDesc.textContent = tText('Impossible de contacter le serveur.');
          withdrawCloseStatusBtn.style.display = 'block';
        }
      });
    }
  ...
  if (typeof socket !== 'undefined' && socket) {
    socket.on('game-invitation-updated', (data) => {
      ...
      const currentUserId = getCurrentUserId();
      const partnerId = Number(data.senderId) === currentUserId ? Number(data.receiverId) : Number(data.senderId);
      if (partnerId) {
        const previewText = getChatMessagePreviewText({ content });
        document.querySelectorAll(`#chat-preview-${partnerId}, #mobile-chat-preview-${partnerId}`).forEach((node) => {
  ...
  setupOpponentRadios.forEach(radio => {
    radio.addEventListener('change', async () => {
      if (radio.value === 'bot') {
        if (setupBotChoiceWrapper) setupBotChoiceWrapper.style.display = 'block';
        if (setupPlayerChoiceWrapper) setupPlayerChoiceWrapper.style.display = 'none';
        loadBotsList();
      } else {
  ...
  const setupGameLivePriceWrapper = document.getElementById('setupGameLivePriceWrapper');
  const gameLiveFreeLabel = document.getElementById('gameLiveFreeLabel');
  const gameLivePaidLabel = document.getElementById('gameLivePaidLabel');
  setupGameLiveModeRadios.forEach(radio => {
    radio.addEventListener('change', () => {
      if (setupGameLivePriceWrapper) {
        setupGameLivePriceWrapper.style.display = (radio.value === 'paid') ? 'block' : 'none';
      }
      if (gameLiveFreeLabel && gameLivePaidLabel) {
        if (radio.value === 'free') {
          gameLiveFreeLabel.classList.add('active');
          gameLivePaidLabel.classList.remove('active');
        } else {
          gameLiveFreeLabel.classList.remove('active');
          gameLivePaidLabel.classList.add('active');
        }
      }
    });
  });
  ...
  let onlineOpponentUsers = [];
  const gamePlayerSearchInput = document.getElementById('gamePlayerSearchInput');
  const gamePlayerSearchResults = document.getElementById('gamePlayerSearchResults');
  const selectedGameOpponentWrapper = document.getElementById('selectedGameOpponentWrapper');
  const selectedGameOpponentAvatar = document.getElementById('selectedGameOpponentAvatar');
  const selectedGameOpponentName = document.getElementById('selectedGameOpponentName');
  const selectedGameOpponentUsername = document.getElementById('selectedGameOpponentUsername');
  const clearSelectedGameOpponentBtn = document.getElementById('clearSelectedGameOpponentBtn');

  const renderGameOpponentSearchResults = (users = [], emptyMessage = 'Aucun joueur en ligne pour le moment.') => {
    ...
    if (filtered.length === 0) {
      gamePlayerSearchResults.innerHTML = `<div style="padding: 10px 14px; font-size: 0.9rem; color: var(--text-muted);">${escapeHtml(emptyMessage)}</div>`;
    ...
    filtered.forEach(user => {
      ...
      row.addEventListener('click', () => {
        ...
        gamePlayerSearchResults.innerHTML = '';
        gamePlayerSearchResults.style.display = 'none';
        gamePlayerSearchInput.value = '';
      });
      gamePlayerSearchResults.appendChild(row);
    });
  ...
  const loadOnlineOpponentUsers = async ({ keepOpen = false } = {}) => {
    ...
    try {
      ...
      console.error('Error loading online opponents:', err);
    }
  };
  ...
  if (clearSelectedGameOpponentBtn) {
    clearSelectedGameOpponentBtn.addEventListener('click', () => {
      selectedOpponentPlayerId = null;
      if (selectedGameOpponentWrapper) selectedGameOpponentWrapper.style.display = 'none';
    });
  }
  ...
  if (startGameBtn) {
    startGameBtn.addEventListener('click', () => {
      ...
      const roundsSelect = document.getElementById('setupGameRounds');
      const rounds = roundsSelect ? parseInt(roundsSelect.value, 10) : 1;

      const liveModeRadio = document.querySelector('input[name="setupGameLiveMode"]:checked');
      const liveMode = liveModeRadio ? liveModeRadio.value : 'free';

      const livePriceInput = document.getElementById('setupGameLivePrice');
      ...
      if (opponentType === 'bot') {
        ...
        if (!selectedOpponentPlayerId) {
          showToast("Veuillez rechercher et sélectionner un adversaire.");
          return;
        }
        opponentId = selectedOpponentPlayerId;
      }
          ...
          if (btn) {
            ...
            btn.addEventListener('click', (e) => {
              ...
              if (btn.classList.contains('spectate-game-action-btn')) {
                const targetGame = window.allLobbyGames?.find(g => g.id === gameId);
                if (targetGame && targetGame.liveMode === 'paid') {
                  showSpectatorPaymentModal(targetGame);
                } else {
                  socket.emit('game-spectate-join', { gameId }, (res) => {
                    if (res && res.error) {
                      showToast(res.error);
  ...
  const updateActiveGameSubtitle = () => {
    ...
    const totalRounds = Number(activeGame.rounds || 1);
    if (totalRounds > 1) {
      const currentRound = Math.min(Number(activeGame.currentRound || 1), totalRounds);
      subEl.textContent = `${modeLabel} • Partie ${currentRound} / ${totalRounds}`;
      return;
    }

    subEl.textContent = modeLabel;
  };
  ...
  const updateScoreboardDisplay = () => {
    ...
    if (activeGame.rounds && Number(activeGame.rounds) > 1) {
      ...
      p2ScoreEl.textContent = String(player2Rounds);
      ...
      p1ScoreEl.textContent = String(activeGame.mathState.scores[1] || 0);
      p2ScoreEl.textContent = String(activeGame.mathState.scores[2] || 0);
    } else {
  ...
  const updateTurnIndicator = () => {
    ...
    if (activeGame.status === 'round-transition') {
      gameTurnStatus.removeAttribute('style');
      const nextRound = Number(activeGame.pendingNextRound || ((activeGame.currentRound || 1) + 1));
      const remainingMs = Math.max(0, Number(activeGame.roundTransitionEndsAt || 0) - Date.now());
      const remainingSeconds = remainingMs > 0 ? Math.ceil(remainingMs / 1000) : null;
      const countdownLabel = remainingSeconds ? ` dans ${remainingSeconds}s` : '';
      gameTurnStatus.textContent = `Victoire validée. Prochaine partie${countdownLabel}...`;
      gameTurnStatus.className = 'game-turn-status-bar waiting';
      if (nextRound > 1) {
        updateActiveGameSubtitle();
      }
  ...
  // --- INVITATIONS AND SPECTATING MODALS ---
  const showGameInviteWaitingModal = (game) => {
    const existing = document.getElementById('gameInviteWaitingModal');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'gameInviteWaitingModal';
    modal.className = 'modal-overlay';
    ...
    content.innerHTML = `
      <div style="position: relative; display: inline-block; margin-bottom: 8px;">
        <img src="${game.player2.avatar || '/assets/avatar_placeholder.jpg'}" style="width: 80px; height: 80px; border-radius: 50%; border: 3px solid var(--primary); object-fit: cover;">
        <span class="pulse-indicator-green" style="position: absolute; bottom: 0; right: 0; width: 16px; height: 16px;"></span>
      </div>
      ...
      <button id="cancelGameInviteBtn" class="settings-secondary-btn" style="width: 100%; justify-content: center; color: #ef4444; border-color: rgba(239, 68, 68, 0.2); background: rgba(239, 68, 68, 0.05); font-weight: 700; cursor: pointer; height: 42px;">
        Annuler l'invitation
      </button>
    `;

    if (!document.getElementById('waiting-bounce-keyframes')) {
      const style = document.createElement('style');
      style.id = 'waiting-bounce-keyframes';
      style.innerHTML = `
        @keyframes bounce {
    ...
    const cancelBtn = content.querySelector('#cancelGameInviteBtn');
    cancelBtn.addEventListener('click', () => {
      socket.emit('game-invite-cancel', { gameId: game.id }, (res) => {
        modal.remove();
  ...
  const showGameInviteReceivedModal = (invite) => {
    ...
    const isPaid = invite.mode === 'paid';
    const betLabel = isPaid
    ...
    modal.innerHTML = `
      <div class="gir-card">
        ...
        <!-- ── Header ── -->
        <div class="gir-header">
          <span class="gir-challenge-label">⚔️ Défi reçu</span>

          <div class="gir-avatar-wrap">
            <div class="gir-avatar-ring"></div>
            <img class="gir-avatar"
                 src="${escapeHtml(invite.challenger.avatar || '/assets/avatar_placeholder.jpg')}"
                 alt="avatar"
        ...
        <div class="gir-body">
          ...
          <p class="gir-invite-text">
            Vous invite à jouer à <strong>${escapeHtml(gameName)}</strong>
          </p>

          <!-- Info pills -->
          <div class="gir-info-grid">
            <div class="gir-info-pill">
              <span class="gir-info-pill-label">🎮 Type de jeu</span>
              <span class="gir-info-pill-value">${escapeHtml(gameName)}</span>
            </div>
            <div class="gir-info-pill">
              <span class="gir-info-pill-label">🔄 Manches</span>
              <span class="gir-info-pill-value">${roundsLabel}</span>
            </div>
            <div class="gir-info-pill" style="grid-column: 1 / -1;">
              <span class="gir-info-pill-label">💰 Mise</span>
          ...
          <div class="gir-timer-wrap">
            <div class="gir-timer-label">
              <span>Temps restant</span>
              <span id="girTimerSeconds">${Math.round(totalMs / 1000)}s</span>
            </div>
            <div class="gir-progress-track">
              <div class="gir-progress-bar" id="gameInviteProgressBar"></div>
            </div>
          </div>
        ...
        <!-- ── Actions ── -->
        <div class="gir-actions">
          <button type="button" id="declineGameInviteBtn" class="gir-btn gir-btn-decline">
            ✕ Refuser
          </button>
          <button type="button" id="acceptGameInviteBtn" class="gir-btn gir-btn-accept">
            ✓ Accepter
          </button>
        </div>
    ...
    const acceptBtn  = modal.querySelector('#acceptGameInviteBtn');
  ...
  const showSpectatorPaymentModal = (game) => {
    const existing = document.getElementById('spectatorPaymentModal');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'spectatorPaymentModal';
    modal.style.cssText = 'position: fixed; inset: 0; background: rgba(0,0,0,0.8); backdrop-filter: blur(12px); display: flex; align-items: center; justify-content: center; z-index: 10000;';

    const card = document.createElement('div');
    card.style.cssText = 'background: var(--bg-card, #1e1e2e); border: 1px solid var(--border-color, rgba(255,255,255,0.1)); border-radius: 20px; padding: 24px; width: 90%; max-width: 400px; text-align: center; color: #fff; box-shadow: 0 10px 30px rgba(0,0,0,0.5);';

    const price = parseFloat(game.livePrice || 0.50).toFixed(2);

    card.innerHTML = `
      <div style="font-size: 3rem; margin-bottom: 12px;">🎟️</div>
      <h3 style="font-size: 1.25rem; font-weight: 700; margin-bottom: 8px;">Live Stream Premium</h3>
      <p style="font-size: 0.95rem; color: var(--text-muted); margin-bottom: 24px; line-height: 1.4;">
        L'accès à ce direct est payant.<br>
        Frais d'entrée : <strong style="color: #22c55e;">${price} $</strong>
      </p>
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
        <button id="cancelSpectateBtn" style="height: 40px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: transparent; color: #fff; font-weight: 600; cursor: pointer;">
          Annuler
        </button>
        <button id="confirmSpectatePayBtn" style="height: 40px; border-radius: 8px; border: none; background: linear-gradient(135deg, #7c4dff 0%, #4f46e5 100%); color: #fff; font-weight: 600; cursor: pointer; box-shadow: 0 4px 10px rgba(124,77,255,0.2);">
          Payer et Rejoindre
        </button>
      </div>
    `;
    ...
    card.querySelector('#cancelSpectateBtn').addEventListener('click', () => modal.remove());
    card.querySelector('#confirmSpectatePayBtn').addEventListener('click', () => {
      modal.remove();
      socket.emit('game-spectate-join', { gameId: game.id }, (res) => {
        if (res && res.error) {
          showToast(res.error);
        } else if (res && res.game) {
          initActiveGame(res.game, true);
  ...
  if (socket) {
    ...
    socket.on('game-started', (game) => {
      ...
      if (isPlayer || (activeGame && activeGame.id === game.id)) {
        ...
        const receivedModal = document.getElementById('gameInviteReceivedModal');
        if (receivedModal) receivedModal.remove();

        initActiveGame(game, window.isSpectatingActiveGame || !isPlayer);
        showGamesView();
      }
    });
    ...
    socket.on('game-invite-received', (invite) => {
      showGameInviteReceivedModal(invite);
    });

    socket.on('game-invite-declined', (data) => {
      const modal = document.getElementById('gameInviteWaitingModal');
      if (modal) modal.remove();
      showToast(data.reason || "L'adversaire a refusé l'invitation.");
    });

    socket.on('game-invite-cancelled', (data) => {
      const modal = document.getElementById('gameInviteReceivedModal');
      if (modal) modal.remove();
      const waitingModal = document.getElementById('gameInviteWaitingModal');
      if (waitingModal) waitingModal.remove();
      showToast(data.reason || "L'invitation a été annulée.");
    });

    socket.on('game-invite-expired', (data) => {
      const modal = document.getElementById('gameInviteReceivedModal');
      if (modal) modal.remove();
      showToast("L'invitation a expiré.");
    });

    socket.on('game-round-over', (data) => {
      const { roundWinnerId, winningStones, nextRound, roundWins, delayMs } = data;
      if (activeGame) {
        activeGame.status = 'round-transition';
        activeGame.pendingNextRound = nextRound;
        activeGame.roundWins = roundWins;
        activeGame.roundTransitionEndsAt = Date.now() + Number(delayMs || 0);
        highlightWinner(winningStones);
        updateTurnIndicator();
        updateScoreboardDisplay();
  ...
  socket.on('p2p-order-message-received', (payload) => {
    ...
    if (senderId && senderId !== currentUserId) {
      triggerNotification(
        'Message Marché P2P',
        message.content || 'Nouveau message sur votre ordre',
  ...
  // Handle hashtag link clicks to show details modal
  document.body.addEventListener('click', async (e) => {
    const hashtagLink = e.target.closest('.hashtag-link');
    if (hashtagLink) {
      e.preventDefault();
      const tagName = hashtagLink.dataset.tag;
      if (!tagName) return;
      ...
      try {
        ...
        if (hashtag && hashtag.name) {
          const modal = document.getElementById('hashtagDetailsModal');
          if (!modal) return;
          
          document.getElementById('detailsHashtagName').textContent = '#' + hashtag.name;
          
          const typeBadge = document.getElementById('detailsHashtagTypeBadge');
          if (hashtag.is_paid) {
            typeBadge.textContent = tText('Premium');
            typeBadge.style.background = '#f59e0b';
          } else {
            typeBadge.textContent = tText('Gratuit');
            typeBadge.style.background = '#10b981';
          }
          
          document.getElementById('detailsHashtagCreatorAvatar').src = hashtag.avatar || '/assets/avatar_placeholder.jpg';
          
          const creatorLink = document.getElementById('detailsHashtagCreatorLink');
          creatorLink.textContent = `${hashtag.first_name || ''} ${hashtag.last_name || ''}`.trim() || hashtag.username || 'Utilisateur';
          creatorLink.href = `/profile/u/${hashtag.username}`;
          
          document.getElementById('detailsHashtagCreatorUsername').textContent = '@' + hashtag.username;
          document.getElementById('detailsHashtagUsageCount').textContent = hashtag.usage_count || 0;
  ...
  // Handle hashtag details modal close
  const hashtagDetailsModal = document.getElementById('hashtagDetailsModal');
  const closeHashtagDetailsModal = document.getElementById('closeHashtagDetailsModal');
  if (closeHashtagDetailsModal && hashtagDetailsModal) {
    closeHashtagDetailsModal.addEventListener('click', () => {
      hashtagDetailsModal.style.display = 'none';
    });
    
    hashtagDetailsModal.addEventListener('click', (e) => {
      if (e.target === hashtagDetailsModal) {
        hashtagDetailsModal.style.display = 'none';
      }
    });
  }
  ...
  // 1. Audio Notification Synthesizer (using Web Audio API for a sweet chime sound)
  ...
  // Trigger sound + optional desktop notification
    ...
    if (document.hidden && 'Notification' in window && Notification.permission === 'granted') {
      try {
        new Notification(title, {
          body: message,
          icon: icon || '/assets/avatar_placeholder.jpg'
        });
      } catch (e) {
  ...
  // Expose playNotificationSound globally so we can trigger it from views if needed
  window.playNotificationSound = playNotificationSound;

  // Request notifications permission on the first user interaction (first click on the document)
  document.addEventListener('click', function initNotificationOnInteraction() {
    requestNotificationPermission();
    document.removeEventListener('click', initNotificationOnInteraction);
  }, { once: true });
  ...
  let deferredPrompt;
  const pwaInstallWrapper = document.getElementById('pwaInstallWrapper');
  ...
  window.addEventListener('appinstalled', (evt) => {
    console.log('TrasX PWA has been installed successfully!');
    if (pwaInstallWrapper) {
      pwaInstallWrapper.style.display = 'none';
    }
